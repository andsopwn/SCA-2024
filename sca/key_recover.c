...
int main() {
    u8 R1[16] = { 0x13, 0x11, 0x1d, 0x7f, 0xe3, 0x94, 0x4a, 0x17, 0xf3, 0x07, 0xa7, 0x8b, 0x4d, 0x2b, 0x30, 0xc5 };
    
    
    u8 R10[16] = { 0xf6, 0x4e, 0x80, 0x0b, 0xd1, 0xf9, 0xf0, 0xa5, 0x23, 0xd5, 0x4c, 0x24, 0xad, 0x02, 0x97, 0xfd };
    u32 v[44] = { 0x00, };

    for(int i = 43 ; i >= 40 ; i--) 
        v[i] = u4byte_in(R10 + ((i % 4) * 4));

    for(int i = 39 ; i >= 0 ; i--) {
        if(i % 4 == 0)  v[i] = (v[i + 4] ^ Rcons[i / 4]) ^ (SubWord(RotWord(v[i + 3])));
        else            v[i] = v[i + 4] ^ v[i + 3];
    }
