...
#define DIR "./traces/"
#define traceFN "samples.npy"
#define ptFN "ciphertext.npy"
#define ctFN "ciphertext.npy"
#define startpt 0
#define endpt 100000

#define TraceLength 125002
#define TraceNum 2500
...
int main()
{
	...
	rfp = fopen(buf, "r");
	if (rfp == NULL) { printf("%s 파일 읽기 오류", ptFN); exit(1); }
...
/*
	...
	for (int i = 0; i < 16 ; i++)
	{
		...
		for (key = 0 ; key < 256; key++) {
			...
			fflush(stdout);
            if(key == 255)
			printf("\r  %02dth Block | KEY[%s%02X%s] CORR[%s%lf%s]\t\t\t\n", i, _grn_, maxkey, _rst_, _red_, maxCorr, _rst_);
			else    
			printf("\r%02dth Block : %s%.1lf%%%s CR[%lf] K[%02X]", i, _red_, ((double)key / 255) * 100, _rst_, maxCorr, maxkey);
			if(key == maxkey) {
				sprintf(buf, "%sct/%02dth.ct", DIR, i);
				wfp = fopen(buf, "wb");
				if (wfp == NULL) { printf("블록 쓰기 에러\n"); exit(1); }
