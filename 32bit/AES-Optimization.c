...
int main(int argc, char* argv[]) {
    u8       MK[32] = { 0x60, 0x3d, 0xed, 0x10, 0x15, 0xca, 0x71, 0xbe, 0x2b, 0x73, 0xae, 0xf0, 0x85, 0x7d, 0x77, 0x81, 0x1f, 0x35, 0x2c, 0x07, 0x3b, 0x61, 0x08, 0xd7, 0x2d, 0x98, 0x10, 0xa3, 0x09, 0x14, 0xdf, 0xf4 };
    ...
    printf("%sMasterKey%s - ", COLOR_RED, COLOR_RESET); for(int i = 0 ; i < keysize / 8 ; i++)    printf("0x%02X, ", MK[i]); puts("\n");
    ...
    finish = clock();
    puts(""); printf("%s[Decrypted vector]%s\n", COLOR_GREEN, COLOR_RESET);
    ...
    for(int i = 0 ; i < 16 ; i++) {
        if(PT[i] != DE[i]) {
            printf("\n\t%s:::::\tWRONG VALUE\t:::::%s\n\t\t%son %d bits%s\n\n", COLOR_RED, COLOR_RESET, BACK_RED, keysize, COLOR_RESET);
            exit(-1);
        }
    }
